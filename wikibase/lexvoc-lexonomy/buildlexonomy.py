...
for term in terms:
	...
	for isolang in langmapping.langcodemapping:
		...
		# get altLabels, according to Lexonomy entry status
		terms[term]['altLabels'][isolang] = []
		if terms[term]['prefLabels'][isolang]['status'] == 'COMPLETED':
			if wikilang in termdata['aliases']: # get prefLabel from LexBib wikibase (preferred, all manual "COMPLETED" translations and LexBib direct label edits are there)
		...
		elif terms[term]['prefLabels'][isolang]['status'] == 'TO CHECK':
		...
		elif terms[term]['prefLabels'][isolang]['status'] == 'AUTOMATIC': # collect everything (lexbib, wikidata, p130-automatic)
