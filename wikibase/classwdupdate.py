...
for c in schemadict:
	...
	for prop in props:
		print('Now beginning update for class '+c+', prop '+prop+'...')
		if prop == "P66": # get en.wikipedia url and write it to LWB using P66
			...
			itemcount = 1
			for item in classmembers:
				print('\nItem ['+str(itemcount)+'], '+str(len(classmembers)-itemcount)+' items left for this prop.')
				wdqid = item['wdid']
				lwbqid = item['lwbid'].replace("http://lexbib.elex.is/entity/","")
						...
						if claim['mainsnak']['snaktype'] == "value":
							if 'qualifiers' in claim:
								if "P582" in claim['qualifiers']:
									continue # skip claims that have an 'end time' P582 qualifier
