...
for corpus in ske_log:
	if 'docs' in ske_log[corpus]:
		...
		for doc in range(len(ske_log[corpus]['docs'])):
			...
			k = requests.get("https://api.sketchengine.eu/bonito/run.cgi/extract_keywords", auth=ske_auth,
			params={
			"attr":"TERM", # for two-word term candidates, or "attr":"lemma" for one-word term candidates, right?
